...
(async()=>{vm.runInContext(core+'\n'+model+'\n'+ui,sandbox,{timeout:5000});await sandbox.__loadPromise;await new Promise(r=>setTimeout(r,10));
 const checks={status:get('status').innerHTML.includes('Live forecast complete'),noError:!get('status').innerHTML.includes('Load error'),cards:(get('cards').innerHTML.match(/class="card /g)||[]).length===8,raws:get('stationRows').innerHTML.includes('Refugio Hills RAWS'),selfTests:get('selfTest').textContent==='8/8',nws:get('nwsCompare').innerHTML.includes('NWS LOX grid max gust')};

...
async function request(url,{method="GET",attempts=3}={}){
  ...
  for(let i=0;i<attempts;i++){
    ...
    try{
      const r=await fetch(url,{method,signal:ctl.signal,headers:{"User-Agent":"Sundowner-Intelligence-SI4-RRFS-F24-Probe/1.0"}});
...
async function main(){
  ...
  for(const c of CASES){
    ...
    const surface=await inspectObject(keyFor(c,"2dfld"),"surface");
  ...
  const surfaceComplete=rows.every(r=>r.surface.head.ok&&r.surface.idx.ok&&Object.values(r.surface.matches).every(v=>v.length>=1));
  const out={
    ...
    source:{provider:"NOAA Open Data Dissemination",bucket:"s3://noaa-rrfs-pds/",lane:"retro_output_final/spring/"},
    rules:{forecast_hour_fixed:24,cases_predeclared_2024_only:true,future_observations_label_only:true,model_tuning_from_2025_forbidden:true,missing_files_remain_missing:true,rrfs_shadow_only:true},

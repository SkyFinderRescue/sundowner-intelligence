...
async function probe(d){const starts=[`https://doi.org/${d.doi}`,`https://data.eol.ucar.edu/dataset/${d.eol}`,`https://data.eol.ucar.edu/codiac/dss/id=${d.eol}`,`https://data.eol.ucar.edu/rest/dataset/${d.eol}`,`https://data.eol.ucar.edu/rest/dataset/list?dataset=${encodeURIComponent(d.eol)}`,`https://data.eol.ucar.edu/rest/dataset/aSearch?keyword=${encodeURIComponent(d.eol)}`];const attempts=[];for(const start of starts)attempts.push(await one(start));return {...d,attempts};}
(async()=>{const datasets=[];for(const d of DATASETS)datasets.push(await probe(d));const out={status:"RESEARCH_ONLY_DO_NOT_LOAD_IN_PRODUCTION",generated:new Date().toISOString(),authority:"NSF NCAR/EOL DOI, dataset application and REST endpoints",rules:{credentials_not_used:true,missing_links_not_invented:true,fire_outcome_used:false,future_observation_leakage:false},datasets};fs.mkdirSync(path.dirname(OUT),{recursive:true});fs.writeFileSync(OUT,JSON.stringify(out,null,2)+"\n");console.log(JSON.stringify(datasets.map(d=>({key:d.key,reachable:d.attempts.some(a=>a.ok),rest_ok:d.attempts.filter(a=>/\/rest\//.test(a.start)).some(a=>a.ok),candidate_links:d.attempts.reduce((n,a)=>n+(a.candidate_links?.length||0),0)})),null,2));if(datasets.some(d=>!d.attempts.some(a=>a.ok)))process.exitCode=2;})().catch(e=>{console.error(e.stack||e);process.exit(1)});

...
const UA={"User-Agent":"Sundowner-Intelligence-SI4-SWEX/1.3","Accept":"application/json,text/html;q=0.9,*/*;q=0.8"};
...
function htmlTitle(html){const m=html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);return m?m[1].replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim():null;}
function formInfo(html,base){const forms=[];const re=/<form\b([^>]*)>([\s\S]*?)<\/form>/gi;let m;while((m=re.exec(html))&&forms.length<20){const attrs=m[1],body=m[2];const am=attrs.match(/action=["']([^"']*)["']/i);const mm=attrs.match(/method=["']([^"']*)["']/i);let action=null;if(am){try{action=new URL(am[1],base).href;}catch{action=am[1];}}const inputs=[];const ir=/<(?:input|select|button)\b([^>]*)>/gi;let im;while((im=ir.exec(body))&&inputs.length<100){const a=im[1];const nm=a.match(/name=["']([^"']+)["']/i);const ty=a.match(/type=["']([^"']+)["']/i);const val=a.match(/value=["']([^"']*)["']/i);if(nm)inputs.push({name:nm[1],type:ty?ty[1]:null,value:val&&val[1].length<200?val[1]:null});}forms.push({action,method:mm?mm[1].toUpperCase():null,inputs});}return forms;}
function fileCandidates(html){const out=new Set();for(const re of [/\b[\w.-]+\.(?:tar\.gz|tgz|nc|cdf|txt|csv|gz|zip)\b/gi,/\/zinc\/file\/download\/[^"'<>\s]+/gi,/\/codiac\/[^"'<>\s]+/gi]){let m;while((m=re.exec(html)))out.add(m[0]);}return [...out].slice(0,500);}
async function probeLegacyCodiac(d){
  const urls=[
    `https://data.eol.ucar.edu/cgi-bin/codiac/fgr_form/id=${encodeURIComponent(d.eol)}`,
    `https://data.eol.ucar.edu/codiac/fgr_form/id=${encodeURIComponent(d.eol)}`,
    `https://data.eol.ucar.edu/codiac/dss/id=${encodeURIComponent(d.eol)}`
  ];
  const out=[];
  for(const url of urls){
    try{
      const r=await fetchRetry(url,{headers:{Accept:'text/html,*/*;q=0.8'}});const html=await r.text();
      out.push({url,status:r.status,ok:r.ok,final_url:r.url,content_type:r.headers.get('content-type'),title:htmlTitle(html),links:hrefs(html,r.url).filter(u=>/(codiac|zinc\/file|download|600\.)/i.test(u)).slice(0,200),forms:formInfo(html,r.url),file_candidates:fileCandidates(html),excerpt:html.replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim().slice(0,1500)});
    }catch(e){out.push({url,status:null,ok:false,error:String(e.message||e)});}
  }
  return out;
}
...
async function datasetApi(d,api){const probes=[];for(const u of [`${api.api_base}dataset/${encodeURIComponent(d.eol)}`,`${api.api_base}dataset?datasetId=${encodeURIComponent(d.eol)}`,`${api.api_base}dataset/aSearch?keyword=${encodeURIComponent(d.eol)}`,`${api.api_base}dataset/aSearch?keyword=${encodeURIComponent(d.query)}`,`${api.api_base}dataset/aSearch?keyword=${encodeURIComponent(d.doi)}`])probes.push(await getJson(u,api.key));let records=probes.filter(p=>p.ok&&p.body?.body);const matches=[];for(const p of probes)matches.push(...idsFromSearch(p.body,d.eol));for(const m of matches.slice(0,3))records.push(await getJson(`${api.api_base}dataset/${encodeURIComponent(m.id)}`,api.key));const urls=[...new Set(records.flatMap(r=>[...collectUrls(r.body)]))];const file_route_probes=await candidateFileRoutes(d,api);const codiac_order_probes=await probeLegacyCodiac(d);return {ok:records.length>0,probes,matches,records,urls,file_route_probes,codiac_order_probes};}
(async()=>{const api=await appInfo();const datasets=[];for(const d of DATASETS)datasets.push({...d,api:await datasetApi(d,api)});const out={status:'RESEARCH_ONLY_DO_NOT_LOAD_IN_PRODUCTION',generated:new Date().toISOString(),authority:'NSF NCAR/EOL zinc REST API + authoritative CODIAC order endpoints',provenance_notes:['NCAR/EOL metadata records identify CODIAC order-form access as the archive access mechanism for orderable EOL datasets.','This probe performs GET-only discovery; it does not submit orders or persist credentials.'],frontend:{script:api.script,api_base:api.api_base,api_key_resolved:Boolean(api.key),api_key_sha256:api.key_hash,route_literals:api.route_literals,route_hints:api.route_hints},rules:{public_frontend_api_only:true,credentials_not_persisted:true,missing_values_not_invented:true,fire_outcome_used:false,future_observation_leakage:false,order_submission:false},datasets};fs.mkdirSync(path.dirname(OUT),{recursive:true});fs.writeFileSync(OUT,JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify({api_key_resolved:Boolean(api.key),route_literals:api.route_literals.length,route_hints:api.route_hints.length,datasets:datasets.map(d=>({key:d.key,record_ok:d.api.records.length>0,urls:d.api.urls.length,file_routes:d.api.file_route_probes.map(p=>p.status),codiac:d.api.codiac_order_probes.map(p=>({status:p.status,final_url:p.final_url,title:p.title,files:p.file_candidates?.length||0,forms:p.forms?.length||0}))}))},null,2));if(!api.key||datasets.some(d=>!d.api.ok))process.exitCode=2;})().catch(e=>{console.error(e.stack||e);process.exit(1)});

...
const UA={"User-Agent":"Sundowner-Intelligence-SI4-SWEX/1.1","Accept":"application/json,text/html;q=0.9,*/*;q=0.8"};
...
function frontendRouteHints(js){
  const hints=new Set();
  const tokens=["download","/file","file/","order","dataset/","aSearch"];
  for(const token of tokens){
    let at=0;
    while((at=js.indexOf(token,at))>=0){
      const lo=Math.max(0,at-180), hi=Math.min(js.length,at+260);
      let s=js.slice(lo,hi).replace(/[\r\n\t]+/g," ").replace(/\s+/g," ");
      s=s.replace(/[A-Za-z0-9_-]{32,}/g,"<redacted-long-token>");
      if(s.length>440)s=s.slice(0,440);
      hints.add(s);
      at+=token.length;
      if(hints.size>=80)break;
    }
    if(hints.size>=80)break;
  }
  return [...hints];
}
async function appInfo(){const landing=await fetchRetry('https://data.eol.ucar.edu/dataset/600.029');const html=await landing.text();const script=hrefs(html,landing.url).find(u=>/\/assets\/.*\.js(?:$|\?)/.test(u));const r=await fetchRetry(script);const text=await r.text();const key=publicApiKey(text);return {script,key,api_base:'https://data.eol.ucar.edu/zinc/rest/',key_hash:key?crypto.createHash('sha256').update(key).digest('hex'):null,route_hints:frontendRouteHints(text)};}
...
async function candidateFileRoutes(d,api){
  const base=api.api_base, id=encodeURIComponent(d.eol);
  const urls=[
    `${base}dataset/${id}/files`,`${base}dataset/${id}/file`,`${base}dataset/${id}/downloads`,`${base}dataset/${id}/download`,
    `${base}file?datasetId=${id}`,`${base}files?datasetId=${id}`,`${base}file/list?datasetId=${id}`,`${base}dataset/files/${id}`,
    `https://data.eol.ucar.edu/zinc/rest/file/list/${id}`,`https://data.eol.ucar.edu/zinc/rest/file/dataset/${id}`
  ];
  const out=[];for(const u of urls)out.push(await probeRoute(u,api.key));return out;
}

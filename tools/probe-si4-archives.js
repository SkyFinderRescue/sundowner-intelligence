...
async function fetchText(url,{method="GET"}={}){
  ...
  try{
    const r=await fetch(url,{method,signal:ctl.signal,headers:{"User-Agent":"Sundowner-Intelligence-SI4-Archive-Probe/1.0"}});
    const text=method==="HEAD"?"":await r.text();
    return {ok:r.ok,status:r.status,url,headers:Object.fromEntries(r.headers),text};
  } finally { clearTimeout(timer); }
}
async function s3List(bucket,prefix,maxKeys=8){
  const url=`https://${bucket}.s3.amazonaws.com/?list-type=2&prefix=${encodeURIComponent(prefix)}&max-keys=${maxKeys}`;
  const r=await fetchText(url);
  const keys=r.ok?xmlKeys(r.text):[];
  return {ok:r.ok&&keys.length>0,status:r.status,url,prefix,keys,error:r.ok&&keys.length===0?"no keys returned":null};
}
async function probe(){
  ...
  const doy=utcDayOfYear(now), hh=String(Math.floor(now.getUTCHours()/1)).padStart(2,"0");
  const rrfsCycle=String(Math.floor(now.getUTCHours()/6)*6).padStart(2,"0");
  ...
  const rrfsPrefixes=[
    `rrfs.${ymd}/${rrfsCycle}/`,
    `rrfs_a/rrfs.${ymd}/${rrfsCycle}/`,
    `rrfs_public/rrfs.${ymd}/${rrfsCycle}/`
  ];
  ...
  for(const product of ["ABI-L2-ACHAC","ABI-L2-ACMC","ABI-L2-LSTC"]){
    checks.goes18_cloud_candidates.push(await s3List("noaa-goes18",`${product}/${yyyy}/${doy}/${hh}/`,4));
  }

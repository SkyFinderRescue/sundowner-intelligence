...
validation-only and are never fixed-lead predictors.
"""
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--input',required=True); ap.add_argument('--out',required=True); ap.add_argument('--target-direction-deg',type=float); ap.add_argument('--dataset-key',choices=['swex_sounding_composite_5hpa','swex_sounding_composite_highres'],required=True); a=ap.parse_args()
 files=[]

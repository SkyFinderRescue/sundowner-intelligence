...
function thresholdForPod(rows,pf,targetPod=.5,target="y"){const events=rows.filter(r=>r[target]);if(!events.length)return null;const candidates=[...new Set(rows.map(pf).filter(Number.isFinite))].sort((a,b)=>b-a);for(const threshold of candidates){const m=classificationMetrics(rows,pf,target,threshold);if(Number.isFinite(m.pod)&&m.pod>=targetPod)return threshold;}return candidates.length?candidates[candidates.length-1]:null;}
function hardNegativeRows(rows){return rows.filter(r=>S.hardNegativeFlag({pressureSupport:r.x[1],mountainWaveScore:r.wave.score,eventObserved:!!r.y}).isHardNegative);}
function falsePositiveRate(rows,pf,threshold){if(!rows.length||!Number.isFinite(threshold))return null;return rows.filter(r=>pf(r)>=threshold).length/rows.length;}
function hardNegativeMetrics(rows,pf,matchedThreshold){const p=rows.map(pf).filter(Number.isFinite);return{n:rows.length,mean_probability:mean(p),p90_probability:quantile(p,.9),max_probability:p.length?Math.max(...p):null,brier_negative_only:rows.length?mean(p.map(x=>x*x)):null,fpr_at_10:falsePositiveRate(rows,pf,.10),fpr_at_20:falsePositiveRate(rows,pf,.20),fpr_at_30:falsePositiveRate(rows,pf,.30),fpr_at_50:falsePositiveRate(rows,pf,.50),train_matched_pod_threshold:matchedThreshold,fpr_at_train_matched_pod:falsePositiveRate(rows,pf,matchedThreshold)};}
...
(async()=>{
  ...
  for(const regime of["western","hybrid","eastern"]){
    const tr=train.byReg[regime],te=test.byReg[regime],m=fit(tr),basePf=r=>r.baseline,candPf=r=>predict(m,r.x),hardTe=hardNegativeRows(te),targetPod=.5,baseThreshold=thresholdForPod(tr,basePf,targetPod),candThreshold=thresholdForPod(tr,candPf,targetPod);
    models[regime]=m;
    report[regime]={baseline:metrics(te,basePf),candidate:metrics(te,candPf),hard_negatives_train:train.hard[regime],hard_negatives_holdout:test.hard[regime],hard_negative_analysis:{selection_rule:"Hard-negative definition is fixed by SI-4 science primitive. Thresholds are selected only on 2024 training rows to reach >=50% training POD, then frozen for 2025 holdout.",target_train_pod:targetPod,baseline:hardNegativeMetrics(hardTe,basePf,baseThreshold),candidate:hardNegativeMetrics(hardTe,candPf,candThreshold),baseline_holdout_at_train_threshold:classificationMetrics(te,basePf,"y",baseThreshold),candidate_holdout_at_train_threshold:classificationMetrics(te,candPf,"y",candThreshold)}};
  }
  report.terrain_response=terrainMetrics(allTest,terrain);
  const out={version:`SI-4-research-${new Date().toISOString().slice(0,10)}`,generated:new Date().toISOString(),status:"RESEARCH_ONLY_DO_NOT_LOAD_IN_PRODUCTION",feature_names:FEATURE_NAMES,upper_air_cache:upper.meta,train:{start:TRAIN_START,end:TRAIN_END,forecast_lead_hours:24},holdout:{start:TEST_START,end:TEST_END,forecast_lead_hours:24},method:"Chronological fixed-lead candidate. SI-3-compatible archived F24 surface/pressure predictors are combined with direct public HRRR archived pressure-level profiles. HADS/RAWS verifying wind is label-only. Fire association is excluded from the Sundowner target. Hard-negative thresholds are selected on training only. Terrain corrections are fit only on training residuals.",models,terrain_response:terrain,report};

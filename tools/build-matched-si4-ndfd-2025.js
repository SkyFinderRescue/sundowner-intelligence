...
function loadUpper(){const x=JSON.parse(fs.readFileSync(UPPER,"utf8"));if(x.status!=="RESEARCH_ONLY_DO_NOT_LOAD_IN_PRODUCTION"||Number(x.forecast_lead_hours)!==24||Number(x.failure_count||0)!==0)throw Error("authoritative upper cache guard failed");const m=new Map();for(const r of x.rows||[]){const t=String(r.valid_time||"").slice(0,13);if(t&&r.zone)m.set(`${t}|${r.zone}`,r.profile);}return m;}
...
(async()=>{
  ...
  for(const n of sourceRows){
    ...
    const time=String(n.target_valid_utc).slice(0,13),profile=toScienceProfile(upper.get(`${time}|${z.name}`));if(profile.length<4){missing.push({key:`${time}|${z.name}`,reason:"missing_upper"});continue;}

...
function prevUrl(lat,lon,varName,lead,model='gfs_hrrr'){const u=new URL('https://previous-runs-api.open-meteo.com/v1/forecast');u.search=new URLSearchParams({latitude:String(lat),longitude:String(lon),start_date:START,end_date:END,hourly:`${varName}_previous_day${lead}`,wind_speed_unit:'mph',timezone:'GMT',models:model});return u}
...
(async()=>{
 ...
 for(const lead of LEADS){
  ...
  for(const z of PAIRS){
   const obs=await hads(z.station),m=await previous(z.lat,z.lon,BASE_VARS,lead,'gfs_hrrr'),mi=makeIndex(m),pairs=[],strongPairs=[],gustErrors=[];
   for(const [t,o] of obs){const i=mi.get(t);if(i==null)continue;const pi=ai.sba.get(t),bi=ai.bfl.get(t),si=ai.smx.get(t);if(pi==null||bi==null||si==null)continue;const val=(v)=>Number(series(m,v,lead)[i]);const sba=Number(series(ap.sba,'pressure_msl',lead)[pi]),bfl=Number(series(ap.bfl,'pressure_msl',lead)[bi]),smx=Number(series(ap.smx,'pressure_msl',lead)[si]);const v={time:t,b:sba-bfl,s:sba-smx,rh:val('relative_humidity_2m'),g:val('wind_gusts_10m'),d:val('wind_direction_10m'),w8:val('wind_speed_850hPa'),d8:val('wind_direction_850hPa'),w7:val('wind_speed_700hPa'),d7:val('wind_direction_700hPa'),sol:val('shortwave_radiation')};if(![v.b,v.s,v.rh,v.g,v.d,v.w8,v.d8,v.w7,v.d7,v.sol].every(Number.isFinite))continue;const p=probability(v,z),sp=strongProbability(v,p,z),actualComp=o.speed*dirComponent(o.dir,z.targetDir),y=actualComp>=20,sy=(Math.max(o.gust||0,o.speed)>=35)&&dirComponent(o.dir,z.targetDir)>.5;pairs.push({p,y});strongPairs.push({p:sp,y:sy});if(Number.isFinite(o.gust)&&Number.isFinite(v.g))gustErrors.push(Math.abs(v.g-o.gust));}

...
async function runCase(name,viewport){
 ...
 try{
  ...
  const artifacts=await page.evaluate(async()=>{const [c,e]=await Promise.all([fetch('./calibration.json',{cache:'no-store'}).then(r=>r.json()),fetch('./data/known-sundowner-events.json',{cache:'no-store'}).then(r=>r.json())]);return{version:c.version||'',events:Array.isArray(e.events)?e.events.length:0}});
  ...
  result.known_events=artifacts.events; if(result.known_events<57)throw Error(`Expected >=57 documented event records, got ${result.known_events}`);

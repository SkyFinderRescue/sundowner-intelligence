...
During the full 2024 HREF member archive on PR #6, all Phase-0 HREF feasibility, field-inventory, exact-F24, member-alignment and point-extraction probes passed. Subsequent full-year extraction attempts identified ten persistently incomplete exact-F24 issuances under the frozen member/point/field contract:
...
The failures were repeated HTTP 404 responses for required frozen HREF member data after bounded retries. The newest verified gap was `2024-11-29T00:00:00.000Z`: the required `hrrr_current` member returned 404 for `u10`, `v10`, surface gust and surface pressure through all five inner retries, and the November monthly job reproduced the failure across all three outer attempts at multiple frozen points. Each outer attempt had already completed more than 2,500 of 2,800 frozen November extraction tasks before reaching the same unavailable issuance.
...
- the 00Z/12Z issuance schedule except the ten explicit availability exclusions;
...
The affected issuances are not replaced with label-selected dates and are not imputed. Missing stays missing. Full-year retained row expectation is therefore `36,000` rather than `36,500`.

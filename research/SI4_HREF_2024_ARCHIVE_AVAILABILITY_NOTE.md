...
During the full 2024 HREF member archive on PR #6, all Phase-0 HREF feasibility, field-inventory, exact-F24, member-alignment and point-extraction probes passed. Subsequent full-year extraction attempts identified thirteen persistently incomplete exact-F24 issuances under the frozen member/point/field contract:
...
The failures were repeated HTTP 404 responses for required frozen HREF member data after bounded retries. The newest verified persistent gap is `2024-11-30T12:00:00.000Z`: the required `hrrr_current` member returned 404 for `u10`, `v10`, surface gust and surface pressure through all five inner retries. In the same November workflow attempt, separate null-status failures occurred on other dates; those are treated as transient infrastructure rather than archive-availability evidence and are not added to the exclusion set. This is the same archive-availability failure class as the previously frozen exclusions, not forecast evidence.
...
- the 00Z/12Z issuance schedule except the thirteen explicit availability exclusions;
...
The affected issuances are not replaced with label-selected dates and are not imputed. Missing stays missing. Full-year retained row expectation is therefore `35,850` rather than `36,500`.

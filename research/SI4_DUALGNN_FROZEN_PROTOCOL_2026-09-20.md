...
14. **Optimizer convention:** the open-access article and supporting-information search reviewed on 2026-09-21 report the learning rate but do not identify an optimizer. To close this implementation degree of freedom before any 2024 outcome score, freeze **Adam** with PyTorch default beta/epsilon settings and the published learning rate **0.03**, with no scheduler unless an authoritative source later proves the published implementation used one before scoring begins. This is an outcome-blind deterministic implementation convention, not a tuned choice. Do not compare optimizers on Sundowner outcomes.
